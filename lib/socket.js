...
class EncryptedSocket extends EventEmitter {
  ...
  _authHost(hostKey) {
    ...
    function receiveAuthSuccess(data) {
      if (Buffer.from(data).equals(SUCCESS)) {
        self._onAuthed()
      }
    }
  ...
  _authPeer(socket, publicKey, secretKey) {
    ...
    function receiveChallengeVerification(decryptedChallenge) {
      if (Buffer.from(challenge).equals(decryptedChallenge)) {
        self.write(SUCCESS)
        self._onAuthed()
      } else {
  ...
  write(data) {
    ...
    debug(`Write ${msg.length} to ${Buffer.from(this.peerKey).toString('hex')}`)
  }
  ...
  _onReceive(data) {
    ...
    debug(`Received ${data.length} from ${Buffer.from(this.peerKey).toString('hex')}`)
