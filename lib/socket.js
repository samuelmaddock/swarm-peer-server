...
class EncryptedSocket extends EventEmitter {
  ...
  write(data) {
    ...
    console.log(`Write ${msg.length} to ${this.peerKey.toString('hex')}`)
  }
  ...
  _onReceive(data) {
    ...
    console.log(`Received ${data.length} from ${this.peerKey.toString('hex')}`)
  ...
  _error(err) {
    console.error(`[EncryptedSocket]`, err)
    this.emit('error', err)
    this.destroy()
  }
}
